...
<script>
  $(document).ready(function () {
    ...
    $('.item-select').select2({ placeholder: 'Select Item', width: '100%' });

    // Update customer info on select
    $('#customerSelect').on('change', function () {
      const selected = $(this).find(':selected');
      $('#nameValue').text(selected.data('name') || ":");
      $('#addressValue').text(selected.data('address') || ":");
      $('#phoneValue').text(selected.data('phone') || ":");
      $('#emailValue').text(selected.data('email') || ":");
    });
